...
class MkldnnPlugin(CWrapPlugin):
    ...
    TYPE_UNPACK = {
        ...
        'mkldnnDataType_t': Template('$arg'),
        'mkldnnEngine_t': Template('$arg'),
...
static PyMethodDef _THMKLDNN_methods[] = {
$methods
  {NULL}
};

PyMethodDef* THMKLDNN_methods()
{
  return _THMKLDNN_methods;
}
""")
...
""")
    ...
    def get_arg_accessor(self, arg, option):
        ...
        if name == 'self':
            ...
            return 'getMkldnnDataType(tensorClass)'
        elif name == 'engine':
            return 'getMkldnnEngine()'

    def process_declarations(self, declarations):
        for declaration in declarations:
            declaration.setdefault('python_name', '_{}'.format(declaration['name']))
            declaration['name'] = 'THMKLDNN_{}'.format(declaration['name'])
            self.declarations.append(declaration)
            for option in declaration['options']:
                for arg in option['arguments']:
                    if arg['name'] in ['self', 'state', 'dataType', 'engine']:
